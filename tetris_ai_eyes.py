...
def main():
    ...
    queue_templates = [[get_bw_mask(cv2.imread('template_images/queue_' + str(i) + '_' + l + '.png')) for l in colors] for i in range(6)]
    ...
    with Listener(on_press=interupt, on_release=None) as listener:  # Create an instance of Listener
        while True:
            ...
            if interrupted:
                ...
                def get_segment(image, upper_left, lower_right, downscale=1):
                    return image[
                        upper_left[0]:lower_right[0]:downscale,
                        upper_left[1]:lower_right[1]:downscale,
                    ]
                ...
                # cv2.imshow('hold_pic', hold_pic)
                # cv2.imshow('board pic', board_pic)
                # cv2.imshow('queue_pic', queue_pic)

                individual_queue_pics = np.array_split(queue_pic, 6, axis=0)
                # for i, pic in enumerate(individual_queue_pics):
                #     cv2.imshow('queue_' + str(i), pic)
                # cv2.waitKey(1)
                ...
                def get_piece_in_image(image, templates, can_be_empty=False):
                    ...
                    if can_be_empty and np.sum(image == 0) > matches[best_match]:
                        ...
                        return colors[best_match]

                hold = get_piece_in_image(hold_mask, hold_templates, True)
                queue = [get_piece_in_image(get_bw_mask(queue_im), queue_idx_templates) for queue_im, queue_idx_templates in zip(individual_queue_pics, queue_templates)]
